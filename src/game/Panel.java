...
public class Panel extends JPanel implements ActionListener {
	 ...
	 */
	private static final int PANEL_WIDTH = (int) Toolkit.getDefaultToolkit().getScreenSize().getWidth();
	private static final int PANEL_HEIGHT = (int) Toolkit.getDefaultToolkit().getScreenSize().getHeight();
	/**
	 ...
	 * podla poziadaviek.
	 */
	public Panel(GridHandler gridHandler, Canvas canvas, MouseInput mouseInput,
			Account account) {
		...
		this.gridHandler.setGrids(GAME_PANEL_WIDTH, PANEL_HEIGHT, CELL_SIZE);
		this.mouseInput.setDimensions(GAME_PANEL_WIDTH, PANEL_HEIGHT, CELL_SIZE);
	...
	public void checkBoxesAction() {
		...
		if (this.bulldozerCheckBoxes.isSelected() && this.mouseInput.isClicked()) {
	...
	public void setCheckBoxesLook() {
		...
		this.bulldozerCheckBoxes.setLook(CellType.EMPTY_CELL);
		this.viewCheckBox.setLook(GridState.OVERGROUND.getImageIcon(),
	...
	private void checkIfAreChecked() {
		...
		for (CheckBox checkBox : this.checkBoxes) {
			if (!this.bulldozerCheckBoxes.isSelected() && !checkBox.isSelected()) {
				counter++;
			}
		}
	...
	private void setUICheckBoxes() {
		...
		for (int i = 0; i < this.checkBoxes.length; i++) {
			this.checkBoxes[i] = new CheckBox(GAME_PANEL_WIDTH + SIDE_PANEL_WIDTH / 4,
					PANEL_HEIGHT * (i + 1) / 7, PANEL_HEIGHT / 8);
			this.add(this.checkBoxes[i]);
		}
		this.bulldozerCheckBoxes = new CheckBox(
				GAME_PANEL_WIDTH + SIDE_PANEL_WIDTH / 4, 0, PANEL_HEIGHT / 8);
		this.add(this.bulldozerCheckBoxes);
		this.viewCheckBox = new CheckBox(
				GAME_PANEL_WIDTH + SIDE_PANEL_WIDTH * 3 / 4, 0, PANEL_HEIGHT / 8);
	...
	private void setUIButtons() {
		...
		for (int i = 0; i < this.gameButtons.length; i++) {
			this.gameButtons[i] = new GameButton(GAME_PANEL_WIDTH + SIDE_PANEL_WIDTH * 3 / 4,
					PANEL_HEIGHT * (i + 1) / 7, PANEL_HEIGHT / 8);
		...
		for (int i = 0; i < this.controlButtons.length; i++) {
			this.controlButtons[i] = new ControlButton(
					...
					PANEL_HEIGHT * 6 / 7, SIDE_PANEL_WIDTH / 2, PANEL_HEIGHT / 14);
	...
	private void setUIThumb() {
		this.thumb = new Thumb(GAME_PANEL_WIDTH + SIDE_PANEL_WIDTH / 4,
				PANEL_HEIGHT * 4 / 7, PANEL_HEIGHT / 9);
		this.add(this.thumb);
	}
	...
	private void setGraph() {
		this.graph = new Graph(GAME_PANEL_WIDTH + SIDE_PANEL_WIDTH * 3 / 4,
				PANEL_HEIGHT * 4 / 7, PANEL_HEIGHT / 9);
		this.add(this.graph);
	}
	...
	private void setUIMoneyBar() {
		this.moneyBar = new MoneyBar(GAME_PANEL_WIDTH + SIDE_PANEL_WIDTH / 2,
				PANEL_HEIGHT * 5 / 7, SIDE_PANEL_WIDTH, PANEL_HEIGHT / 12);
