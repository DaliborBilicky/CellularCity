  ...
  public void drawGrid(int width, int height) {
    for (int i = 0; i <= width / this.cellSize; i++) {
      for (int j = 0; j <= height / this.cellSize; j++) {
        this.graphics.drawLine(i * this.cellSize, 0, i * this.cellSize,
                               height - (height % this.cellSize));
        this.graphics.drawLine(0, j * this.cellSize,
  ...
  public void drawGridWithInfra(CellType[][] grid) {
    for (int i = 0; i < grid.length; i++) {
      for (int j = 0; j < grid[i].length; j++) {
        this.graphics.drawImage(grid[i][j].getBufferedImage(),
                                this.cellSize * j, this.cellSize * i,
                                this.cellSize, this.cellSize, null);
      }
    }
  }
